...
export function ProductList({ data, filters, onEdit }: ProductListProps) {
  ...
  const filteredData = data.filter((item) => {
    ...
    if (filters.isDG !== null && item.isDG !== filters.isDG) {
      return false;
    }
    if (filters.dgClass.length > 0 && item.dgClass && !filters.dgClass.includes(item.dgClass)) {
      return false;
    }
    return true;
  });
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-[#F1F0FB] border-b border-gray-200">
              ...
              <TableHead className="text-dgxprt-navy font-semibold">Pack Group</TableHead>
              <TableHead className="text-dgxprt-navy font-semibold">UN Number</TableHead>
          ...
          <TableBody>
            {paginatedData.map((item) => (
              ...
              >
                ...
                <TableCell>{item.packGroup || "-"}</TableCell>
                <TableCell>{item.unNumber || "-"}</TableCell>
